...
public interface OrderDao extends PagingAndSortingRepository<Order, Integer>, JpaSpecificationExecutor<Order>{
	...
	@Query("SELECT o FROM Order o WHERE orderSnMain=?1 AND shippingId=?2")
	List<Order> findByOrderSnMainAndShippingId(String orderSnMain,int shippingId);
	...
	@Transactional
	@Modifying
	@Query("UPDATE Order set needInvoice = ?2 where orderSnMain = ?1")
	int updateNeedInvoiceByOrderSnMain(String orderSnMain, int needInvoice);
}
