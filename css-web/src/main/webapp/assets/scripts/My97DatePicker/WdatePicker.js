...
function my97datepicker(dateFilter) {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function (scope, element, attrs, ngModelCtrl) {
            var format=attrs["my97datepicker"];
            var datepickerconfig = eval('(' + format + ')');
            var onpicked = datepickerconfig.onpicked;
            var oncleared = datepickerconfig.oncleared;
            var setcurrentDate = function (date) {
                ngModelCtrl.$setViewValue(date);
                scope.$apply();
            };
            datepickerconfig.onpicked = function (data) {
                setcurrentDate(data.cal.getNewDateStr());
                if (onpicked) {
                    onpicked(data);
                }
            };
            datepickerconfig.oncleared = function (data) {
                setcurrentDate(null);
                if (oncleared) {
                    oncleared(data);
                }
            ...
            element.bind("focus", function (event) {
                WdatePicker(datepickerconfig);
            });

            ngModelCtrl.$formatters.push(function(data) {
                if(data){
                    data = dateFilter(data,datepickerconfig.dateFmt);
                }
...
app.directive("my97datepicker", my97datepicker);